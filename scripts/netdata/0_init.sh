...
# armel does *not* have nodejs. debian/ubuntu dither on netcat.
case "${PACKER_BUILD_NAME}" in
  sheeva)      extrapkgs=('netcat-openbsd')  ;;
  espressobin) extrapkgs=('nodejs' 'netcat-openbsd') ;;
  *)           extrapkgs=('nodejs' 'netcat') ;;
esac

# install netdata - omnibus install
apt-get install autoconf autoconf-archive autogen automake curl cmake gcc git gzip libjudy-dev libelf-dev liblz4-dev libmnl-dev libssl-dev libuv1-dev lm-sensors make pkg-config python3 python3-mysqldb python3-psycopg2 python3-pymongo tar uuid-dev zlib1g-dev jq firewalld g++ "${extrapkgs[@]}"
